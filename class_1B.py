...
server = app.server # tell Gunicorn or sever to run on server
...
def get_upload_summary(selection, uploaded_files,  names_of_files):
    global dataframe
    if (uploaded_files and names_of_files) is not None and dataframe is None:
        ...
        start_year_sel =  selection[0]
        end_year_sel = selection[1]
        tbl = etl.fromdataframe(dataframe)
        tbl = etl.convert(tbl, 'year', int)
        start_year, end_year =  etl.limits(tbl, 'year')
        filtered_df =  dataframe[  dataframe['year']>= start_year_sel ]
        filtered_df =  filtered_df [  filtered_df ['year'] <= end_year_sel ]
        figure = ex.bar(filtered_df, x='month', y='sales', color='year')

        return '', figure, start_year, end_year, selection, f'selection: {start_year_sel} - {end_year_sel}'
